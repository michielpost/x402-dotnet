...
namespace x402.Client.Tests
{
    public class PaymentRequiredHandlerV1Tests
    {
        ...
        public async Task Adds_X_PAYMENT_Header_On_Retry_When_402_With_Accepts()
        {
            ...
            var handler = new PaymentRequiredV1Handler(new WalletProvider(wallet), new NullLogger<PaymentRequiredV1Handler>(), maxRetries: 2)
            {
                InnerHandler = inner
            };
        ...
        public async Task No_Retry_When_Accepts_Empty()
        {
            ...
            var handler = new PaymentRequiredV1Handler(new WalletProvider(wallet), new NullLogger<PaymentRequiredV1Handler>(), maxRetries: 3)
            {
                InnerHandler = inner
            };
            var client = new HttpClient(handler);
        ...
        public async Task Respects_MaxRetries_When_Still_402()
        {
            ...
            var handler = new PaymentRequiredV1Handler(new WalletProvider(wallet), new NullLogger<PaymentRequiredV1Handler>(), maxRetries: 1)
            {
                InnerHandler = inner
            };
            var client = new HttpClient(handler);
